...
//--- PUBLIC ---//
TextureGL::TextureGL(const std::string& filePath, const bool needToFlip)
	: filePath_(filePath)
{
...
#ifdef _DEBUG
	std::cout << "Loading TextureGL: " << pathToTexture << "\n";
#endif
...
TextureGL::TextureGL(const std::array<std::string_view, 6>& textures, const bool needToFlip)
{
	stbi_set_flip_vertically_on_load(needToFlip);
...
uint32_t TextureGL::ID() const
{
	return id_;
} // end of getID

void TextureGL::setWrapRepeat() const
{
	glTextureParameteri(id_, GL_TEXTURE_WRAP_S, GL_REPEAT);
	glTextureParameteri(id_, GL_TEXTURE_WRAP_T, GL_REPEAT);
} // end of setWrapRepeat()

void TextureGL::setNoMipmapsLinear() const
{
	// disable mipmaps
	glTextureParameteri(id_, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTextureParameteri(id_, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
...
int32_t TextureGL::getWidth() const
{
	return width_;
} // end of getWidth()

int32_t TextureGL::getHeight() const
{
	return height_;
} // end of getHeight()
